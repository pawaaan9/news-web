    ...
    if (id) {
      ...
      if (currentArticle) {
        const related = allNewsArticles
          .filter(item => item.category === currentArticle.category && item.id !== id)
          .slice(0, 2)
          .map(item => ({
            ...item,
    ...
    <main>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="text-xs mb-4">
          <Link href="/" className="text-blue-500 hover:underline">මුල් පිටුව</Link> &gt; 
          <Link href={`/?category=${article.category}`} className="text-blue-500 hover:underline ml-1">{article.category}</Link> &gt; 
          <span className="text-gray-700 ml-1">{article.title.substring(0, 20)}...</span>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center text-sm text-gray-600 mb-4">
            <span className="font-medium">{article.author}</span> විසින් • {article.date}
          </div>
        </div>
              ...
              {relatedNews.map((news) => (
                <Link href={`/news/${news.id}`} key={news.id} passHref>
                  <div className="bg-white rounded-lg overflow-hidden shadow-md border border-gray-200 cursor-pointer hover:shadow-lg transition-shadow duration-300">
                    ...
                    <div className="p-3">
                      <h3 className="text-sm font-semibold mb-1 leading-snug">{news.title}</h3>
                      <p className="text-xs text-gray-600 mb-2 line-clamp-3">{news.description}</p>
                      <div className="text-[10px] text-gray-500">
                        <span className="font-medium">{news.author}</span> විසින් • {news.date}
                      </div>
                    </div>
                  </div>
                </Link>
